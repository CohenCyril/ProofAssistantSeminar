...
Proof. by move=> A B C ab bc /ab /bc. Restart. tauto. Qed.

(* EXERCISE: the contrapositive.  Prove manually or use [tauto]. *)
Goal forall A B : Prop, (A -> B) -> ~B -> ~A.
Proof. by move=> A B ab bF /ab /bF. Restart. tauto. Qed.
...
*)
  assert (forall x, P1 x -> P5 x) by firstorder.
  by firstorder.
Qed.
...
Goal forall a b c : Z, a <= b -> 0 <= b <= c -> a <= c + b.
Proof. lia. Qed.

Close Scope Z_scope. (* back to nat scope, one may also use [%N]*)
Goal forall a b c : nat, b <= a -> b <= c -> a - b + c = a + (c - b).
Proof. lia. Qed.
...
Proof. by move=> opxy; rewrite -[e]opxy op_e. Qed.

Goal forall x : nat, (forall y, x + y = y) -> x = 0.
Proof. exact: e_unique. Qed.
...
Proof. by rewrite !opA [op x y]opC. Qed.

Goal forall n m k : nat, n + (m + k) = m + (n + k).
Proof. exact: opCA. Qed.
...
Lemma inv_eq (G : Type) `{Group G} (x y : G) : op x y = e -> inv x = y.
Proof.
move=> /(congr1 (op (inv x)))/esym.
by rewrite op_e opA inv_op e_op.
Qed.
...
Proof.
apply/inv_eq.
by rewrite !opA -[op (op x y) _]opA op_inv op_e op_inv.
Qed.
...
Proof.
by refine {|
   op := fun f g => {|fn := fun x => fn f (fn g x)|};
   e := {| fn := fun x => x |}
|}.
Qed.
...
Proof. by move=> *; rewrite expnD. Qed.
...
Proof. by move=> n; rewrite dvd1n. Qed.

Goal forall d : nat, d %| 0.
Proof. by move=> d; exact: dvdn0. Qed.
...
Lemma dvdn_lincomb (d m n a b : nat) :
  d %| m -> d %| n -> d %| a * m + b * n.
Proof. by move=> dm dn; rewrite dvdn_add// dvdn_mull. Qed.
...
Lemma dvdn_mul_mul a b m n :
  a %| m -> b %| n -> a * b %| m * n.
Proof.
by move=> /dvdnP[p ->] /dvdnP[q ->]; rewrite mulnACA dvdn_mull.
Qed.
...
Proof. by move=> n; rewrite big1_eq. Qed.
...
Proof. move=> n m f; exact: exchange_big. Qed.
...
Proof.
move=> n m f; under eq_bigr do rewrite big_split sum_nat_const card_ord //=.
by rewrite big_split/= sum_nat_const card_ord -big_distrr/=.
Qed.
...
Proof.
elim: n => [|n IHn]; first by rewrite big_ord1.
by rewrite big_ord_recr/= IHn; lia.
Qed.
...
Lemma sum_odd n : \sum_(i < n) (2 * i + 1) = n ^ 2.
Proof.
case: n => [|n]; first by rewrite big_ord0.
rewrite big_split/= -big_distrr/= gauss sum_nat_const card_ord.
rewrite muln_divA ?two_dvd_consecutive//.
lia.
Qed.
