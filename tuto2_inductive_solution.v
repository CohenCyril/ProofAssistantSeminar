...
Fixpoint append (A: Type) (l1 l2: list A): list A := match l1 with
  | [] => l2
  | x :: l1' => x :: (append A l1' l2)
end.
...
Definition head (A: Type) (l: list A): option A := match l with
  | [] => None
  | x :: l' => Some x
end.
...
Fixpoint size (t: BTree): nat := match t with
  | Empty => 0
...
Proof.
  ...
  rewrite Nat.add_0_r.
  apply S_plus_lt.
    apply (Nat.lt_le_trans (size t1) (2 ^ height t1)).
      assumption.
    apply Nat.pow_le_mono_r.
      discriminate.
    apply Nat.le_max_l.
  apply (Nat.lt_le_trans (size t2) (2 ^ height t2)).
    assumption.
  apply Nat.pow_le_mono_r.
    discriminate.
  apply Nat.le_max_r.
